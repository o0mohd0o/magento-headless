define([
	...
	return {
		controller: function($scope) {
			...
			$scope.$on('cloneDynamicItem', function(event, item) {
				var index = $scope.model[$scope.options.key].indexOf(item);
				if (index !== -1) {
					var clonedItem = angular.copy(item);
					clonedItem['position'] = $scope.model[$scope.options.key].length + 1;
					$scope.model[$scope.options.key].splice(index + 1, 0, clonedItem);
				}
			});
